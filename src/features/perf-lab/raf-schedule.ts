...
export function rafSchedule<T extends (...args: never[]) => void>(
  ...
  // TODO 1: You need two pieces of state: the frame handle you asked for, and the call you are holding for it.
  // TODO 2: Request a frame only when one is not already pending. Requesting per call is the bug this function exists to remove.
  // TODO 3: Overwrite the held call on every call. The frame should run with the newest scroll position, not the oldest.
  // TODO 4: `fn.bind(this, ...args)` captures the receiver and the arguments in one value, which is tidier than keeping both by hand.
  // TODO 5: Clear the handle *before* invoking — a handler that schedules itself again must be able to.
  // TODO 6: `cancel()` has to clear both the handle and the held call, or a cancelled frame leaks its payload into the next one.
